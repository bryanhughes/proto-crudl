/* Get the schema for many to many relationships starting with a left table */
SELECT DISTINCT
	l_tc.constraint_schema left_schema,
	l_tc.table_name left_table,
	l_kcu.column_name left_column,
	j_tc.constraint_schema join_schema,
	j_tc.table_name join_table,
	j_kcu.column_name join_column,
	r_rel_kcu.table_schema right_schema,
	r_rel_kcu.table_name right_table,
	r_kcu.column_name right_column
FROM
...
WHERE
	...
	AND l_tc.table_name = 'example_a'
	AND j_kcu.column_name = r_rel_kcu.column_name
ORDER BY
	join_schema, join_table, right_schema, right_table
