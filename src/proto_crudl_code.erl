...
%% API
-export([generate/3, build_enum_case/1, is_version/2, is_excluded/2, has_timestamps/1, generate_utils/1]).
...
generate_utils(OutputConfig) ->
    Header = "%%-------------------------------------------------------------------\n"
             "%% This file is automatically generated and includes proto_crudl utilities.\n"
             "%% ---- DO NOT MAKE CHANGES DIRECTLY TO THIS FILE! ----\n\n"
             "-module(proto_crudl_utils).\n\n",

    SourcePath = proplists:get_value(source_path, OutputConfig, "src"),
    FullPath = filename:join([SourcePath, "proto_crudl_utils.erl"]),
    case filelib:ensure_dir(FullPath) of
        ok -> ok;
        {error, eexist} -> ok;
        {error, eacces} ->
...
to_string({{Y, Mo, D}, {H, Mn, _S}}) ->
    ...
    io_lib:format(FmtStr, [Y, Mo, D, H, Mn]);
...
",
    ok = file:write_file(FullPath, Out, [append]).
