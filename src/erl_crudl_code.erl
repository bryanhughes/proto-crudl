...
generate_sources(Provider, SourcePath, Suffix, UseRecords, UsePackage, UseGpb, Database, [{_Key, Table} | Rest]) ->
    S = erl_crudl_utils:to_string(Table#table.schema),
    N = erl_crudl_utils:to_string(Table#table.name),
    ...
    case {UseRecords, UseGpb} of
        {true, true} ->
            ...
            erl_crudl_records:generate_functions(Provider, FullPath, UsePackage, Table);
        {true, false} ->
            io:format("~p (using_records)~n", [FullPath]),
            generate_exports(use_records, FullPath, Table),
...
generate_record(FullPath, UsePackage, #table{schema = S, name = T, select_list = SL, columns = ColDict}) ->
    RecordStr = case UsePackage of true ->
                        "-record('" ++ erl_crudl_utils:to_string(S) ++ "." ++ erl_crudl_utils:camel_case(T) ++ "', {";
                    _ ->
                        "-record('" ++ erl_crudl_utils:camel_case(T) ++ "', {"
                end,
...
write_record_fields(FullPath, 0, StrLen, [ColumnName | Rest], ColDict, Acc) ->
    ...
    CName = erl_crudl_utils:to_string(Column#column.name),
    UdtName = erl_crudl_psql:sql_to_erlang_datatype(Column#column.udt_name),
...
generate_defines(FullPath, S, N, Table) ->
    ...
    [ok = file:write_file(FullPath, "-define(" ++ erl_crudl_utils:to_string(LookupName) ++ ", \"" ++ LookupStr ++ "\").\n",
...
generate_enums(FullPath, [{_Key, #column{name = Name, valid_values = VV}} | Rest]) when length(VV) > 0 ->
    ...
    EnumMapStr = lists:join(", ", [io_lib:format("'~s' => <<\"~s\">>", [erl_crudl_proto:make_enum_label(V), erl_crudl_utils:to_string(V)]) || V <- VV]),
    file:write_file(FullPath, io_lib:format("-define(~s_ENUMS, #{~s}).\n", [erl_crudl_utils:camel_case(Name), EnumMapStr]), [append]),

    ValueMapStr = lists:join(", ", [io_lib:format("<<\"~s\">> => '~s'", [erl_crudl_utils:to_string(V), erl_crudl_proto:make_enum_label(V)]) || V <- VV]),
    file:write_file(FullPath, io_lib:format("-define(~s_VALUES, #{~s}).\n", [erl_crudl_utils:camel_case(Name), ValueMapStr]), [append]),
...
generate_exports(RecordsOrMaps, FullPath, Table) ->
    ...
    case ["         " ++ erl_crudl_utils:to_string(Idx#index.name) ++ "/1"
    ...
    case ["         " ++ erl_crudl_utils:to_string(Idx#index.name) ++ "/3"
    ...
    case ["         " ++ atom_to_list(FunName) ++ "/" ++ integer_to_list(erl_crudl_psql:count_params(FunSql))
    ...
    Fun = fun({_Key, Column}, Acc) ->
                case length(Column#column.valid_values) of
                    0 ->
                        Acc;
                    _ ->
                        ["         " ++ erl_crudl_utils:to_string(Column#column.name) ++ "_enum/1,\n" ++
                         "         " ++ erl_crudl_utils:to_string(Column#column.name) ++ "_value/1,\n" | Acc]
                end
          end,
...
build_select_with_limit_sql(Schema, Name, #table{columns = ColDict, select_list = SelectList}) ->
    ...
    lists:flatten("SELECT " ++ lists:join(", ", lists:append(Clause1, Clause2)) ++
                  " FROM " ++ erl_crudl_utils:to_string(Schema) ++ "." ++ erl_crudl_utils:to_string(Name) ++
                  " LIMIT $1 OFFSET $2").
...
build_select_clause([ColumnName | Rest], ColDict, Acc) ->
    ...
    case Column#column.data_type of
        ...
        _ ->
            build_select_clause(Rest, ColDict, [erl_crudl_utils:to_string(ColumnName) | Acc])
    end.
    ...
    case Column#column.select_xform of
        ...
        Operation ->
            build_select_xforms(Rest, ColDict,
                                [erl_crudl_utils:to_string(Operation) ++ " AS " ++ erl_crudl_utils:to_string(ColumnName) | Acc])
    end.
...
bind_select_params(Cnt, [Name | Rest], Acc) ->
    bind_select_params(Cnt + 1, Rest, [erl_crudl_utils:to_string(Name) ++ " = $" ++ integer_to_list(Cnt) | Acc]).


-spec build_select_map(#table{}) -> string().
build_select_map(#table{pkey_list = Pkeys}) ->
    PKColumns = lists:reverse([erl_crudl_utils:to_string(Pkey) ++ " := " ++
                               erl_crudl_utils:camel_case(erl_crudl_utils:to_string(Pkey)) || Pkey <- Pkeys]),
...
build_select_params(#table{pkey_list = Pkeys}) ->
    PKColumns = lists:reverse([erl_crudl_utils:camel_case(erl_crudl_utils:to_string(Pkey)) || Pkey <- Pkeys]),
    lists:flatten("[" ++ lists:join(", ", PKColumns) ++ "]").
...
build_lookup_list_sql(Table, Index) ->
    Schema = erl_crudl_utils:to_string(Index#index.table_schema),
    Name = erl_crudl_utils:to_string(Index#index.table_name),
    ColDict = Table#table.columns,
    SelectList = Table#table.select_list,
...
build_lookup_list_map(undefined, LookupColumns) ->
    Columns = lists:reverse([erl_crudl_utils:to_string(C) ++ " := " ++
                             erl_crudl_utils:camel_case(erl_crudl_utils:to_string(C)) || C <- LookupColumns]),
...
build_list_params(LookupColumns) ->
    Columns = [erl_crudl_utils:camel_case(erl_crudl_utils:to_string(C)) || C <- LookupColumns],
...
build_delete_sql(Schema, Name, Table) ->
    ...
    {_Cnt, PKColumns1} = bind_select_params(1, PKColumns, []),
    lists:flatten("DELETE FROM " ++ erl_crudl_utils:to_string(Schema) ++ "." ++ erl_crudl_utils:to_string(Name) ++
                  " WHERE " ++ lists:join(" AND ", PKColumns1)).
...
build_update_sql(Schema, Name, Table) ->
    ...
    lists:flatten("UPDATE " ++ erl_crudl_utils:to_string(Schema) ++ "." ++ erl_crudl_utils:to_string(Name) ++
...
build_update_clause(Cnt, [ColumnName | Rest], ColDict, CAcc) ->
    Cname = erl_crudl_utils:to_string(ColumnName),
    Column = orddict:fetch(ColumnName, ColDict),
...
build_update_xforms(ColDict, [ColumnName | Rest], Pos, Clause) ->
    ...
    case Column#column.update_xform of
        ...
        Operation ->
            ...
            {NewPos, NewFun} = rewrite_xform_fun(Pos, erl_crudl_utils:to_string(Operation)),
            Cname = erl_crudl_utils:to_string(ColumnName),
...
build_update_map(#table{select_list = SelectList}) ->
    UpdateColumns = lists:reverse([erl_crudl_utils:to_string(C) ++ " := " ++
                                   erl_crudl_utils:camel_case(erl_crudl_utils:to_string(C)) || C <- SelectList]),
...
build_update_record(RecordName, #table{select_list = SelectList}) ->
    UpdateColumns = [erl_crudl_utils:to_string(C) ++ " = " ++
                     erl_crudl_utils:camel_case(erl_crudl_utils:to_string(C)) || C <- SelectList],
...
build_insert_sql(S, N, Table) ->
    ...
    lists:flatten("INSERT INTO " ++ erl_crudl_utils:to_string(S) ++ "." ++ erl_crudl_utils:to_string(N) ++
    ...
    case {Column#column.is_sequence, Column#column.data_type} of
        ...
        {false, _} ->
            build_insert_clause(Cnt + 1, Rest, ColDict,
                                [erl_crudl_utils:to_string(ColumnName) | CAcc],
    ...
    case Column#column.insert_xform of
        ...
        Operation ->
            ...
            build_insert_xforms(ColDict, Rest, NewPos,
                                [erl_crudl_utils:to_string(ColumnName) | Clause],
                                [NewFun | Params])
    end.
...
build_insert_defaults_sql(Schema, Name, #table{columns = ColDict,
    ...
    lists:flatten("INSERT INTO " ++ erl_crudl_utils:to_string(Schema) ++ "." ++ erl_crudl_utils:to_string(Name) ++
                  " (" ++ lists:join(", ", Clause2) ++ ") VALUES (" ++ lists:join(", ", Params2) ++ ")" ++
                  " RETURNING " ++ lists:join(", ", lists:append(RetClause1, RetClause2))).
...
build_insert_map(#table{insert_list = InsertList, default_list = DefaultList}) ->
    InsertColumns = lists:reverse([erl_crudl_utils:to_string(C) ++ " := " ++
                                   erl_crudl_utils:camel_case(erl_crudl_utils:to_string(C)) || C <- InsertList]),
    InsertColumnsWODefault = lists:reverse([erl_crudl_utils:to_string(C) ++ " := " ++
                                            erl_crudl_utils:camel_case(erl_crudl_utils:to_string(C)) || C <- InsertList,
...
build_insert_record(RecordName, #table{insert_list = InsertList, default_list = DefaultList}) ->
    InsertColumns = [erl_crudl_utils:to_string(C) ++ " = " ++
                     erl_crudl_utils:camel_case(erl_crudl_utils:to_string(C)) || C <- InsertList],
    Fun = fun(C, Acc) ->
            case lists:member(C, DefaultList) of
                true -> [erl_crudl_utils:to_string(C) ++ " = " ++"undefined" | Acc];
                _ -> [erl_crudl_utils:to_string(C) ++ " = " ++ erl_crudl_utils:camel_case(erl_crudl_utils:to_string(C)) | Acc]
            end
          end,
...
handle_mapped_params(ColDict, [CName | Rest], Acc) ->
    ...
    case length(C#column.valid_values) of
        0 ->
            case erl_crudl_psql:is_timestamp(C#column.udt_name) of
                true ->
                    handle_mapped_params(ColDict, Rest, ["ts_decode_map(" ++ erl_crudl_utils:camel_case(CName) ++ ")" | Acc]);
                false ->
                    handle_mapped_params(ColDict, Rest, [erl_crudl_utils:camel_case(erl_crudl_utils:to_string(CName)) | Acc])
            end;
        _ ->
            handle_mapped_params(ColDict, Rest, [erl_crudl_utils:to_string(CName) ++ "_value(" ++
                                                 erl_crudl_utils:camel_case(CName) ++ ")" | Acc])
    end.
...
rewrite_func_params(Pos, Fun, [{char, 1, Chr}, {atom, 1, Remaining} | Rest]) ->
    ...
    Name = erl_crudl_utils:to_binary(lists:flatten([Chr, atom_to_list(Remaining)])),
...
rewrite_func_params(Pos, Fun, [{char, 1, Chr} | Rest]) ->
    Name = erl_crudl_utils:to_binary(lists:flatten([Chr])),
    NewFun = lists:flatten(string:replace(Fun, "$" ++ Name, "$" ++ integer_to_list(Pos))),
...
build_enum_cases([{_Key, #column{name = N, valid_values = VV}} | Rest], Acc) when length(VV) > 0 ->
    Name = erl_crudl_utils:camel_case(N),
    UpperSnakeCase = erl_crudl_utils:capitalize(Name),
    Len = length(Name) + 12,
    Str = "    " ++ Name ++ "Value = case maps:get(" ++ Name ++ ", ?" ++ UpperSnakeCase ++ "_ENUMS) of\n" ++
          erl_crudl_utils:spaces(Len + 4) ++ "{badkey, Key} ->\n" ++
          erl_crudl_utils:spaces(Len + 8) ++ "{error, {badkey, Key}};\n" ++
          erl_crudl_utils:spaces(Len + 4) ++ "Value__ ->\n" ++
          erl_crudl_utils:spaces(Len + 8) ++ "Value__\n" ++
          erl_crudl_utils:spaces(Len) ++ "end,\n",
...
define_test() ->
    ...
    {ok, Database1} = erl_crudl:process_configs(Configs, Database),
